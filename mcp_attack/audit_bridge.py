...
def load_audit_rule_ids(audit_path: str) -> Dict[str, Any]:
    ...
    return {"finding_rule_ids": finding_rule_ids, "p2_matrix_ids": p2_matrix_ids, "p2_matrix": p2_matrix}


def filter_variants_by_audit(variants: List[AttackVariant], audit_path: str,
                             mode: str = "filter") -> Tuple[List[AttackVariant], List[str]]:
    """mode='filter': keep only variants whose rule_ids intersect the audit's finding rule_ids.
    mode='prioritize': keep all variants, reorder matched ones first.
    Falls back to the full, unfiltered catalog (with a limitation note) when
    the audit has no findings to filter by."""
    limitations: List[str] = []
    info = load_audit_rule_ids(audit_path)
    rule_ids = info["finding_rule_ids"]
    if not rule_ids:
