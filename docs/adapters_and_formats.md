...
| `source_snapshot` | дерево исходников (ast-скан деклараций `@…tool`, проверка потоков профиля регулярными выражениями) или заранее извлечённый файл фактов | что имя функции определяет её побочные эффекты; код не исполняется |
