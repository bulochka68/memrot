...
If the ``mcp-scan`` binary is on PATH (``pip install mcp-scan``) it is run
against the config file and its JSON output is attached to the audit under
``definition_analysis.mcp_scan``.  Any issue it reports is converted into a
definition-plane finding.  When the tool is missing the audit records that
and continues - the built-in linter still runs.
"""
...
from ..models import Finding, Provenance, Risk

_SEVERITY_MAP = {
    "critical": Risk.CRITICAL, "high": Risk.HIGH, "medium": Risk.MEDIUM, "low": Risk.LOW,
    "error": Risk.HIGH, "warning": Risk.MEDIUM, "info": Risk.LOW,
}
...
def run_mcp_scan(config_path: str, timeout: float = 120.0, binary: str = "mcp-scan") -> Tuple[Dict[str, Any], List[Finding]]:
    ...
    if not exe:
        return {"available": False, "reason": f"{binary} not found on PATH"}, []
...
def _convert(data: Any) -> List[Finding]:
    """Best-effort conversion; mcp-scan's JSON layout has changed across versions."""
        ...
        if isinstance(node, dict):
            ...
            if isinstance(issues, list):
                for issue in issues:
                    ...
                    sev = _SEVERITY_MAP.get(str(issue.get("severity") or issue.get("level") or "high").lower(), Risk.HIGH)
                    code = str(issue.get("code") or issue.get("type") or issue.get("category") or "ISSUE").upper()
                    findings.append(Finding(
                        id=f"MCPSCAN-{code}", type=f"MCP_SCAN_{code}", severity=sev,
                        title="mcp-scan finding",
                        description=str(issue.get("message") or issue.get("description") or issue),
                        plane="definition", provenance=Provenance.EFFECTIVE,
                        server=srv, tool=tl, evidence={"raw": issue},
                    ))
